...
- **Production institutional use is the load-bearing alpha validation.** An institutional customer has built a full fund-management application on Remy at alpha, set to run that organization's fund operations, with ongoing rollout underway. The application was produced without engineering support beyond the two founders — a real institutional deployment on a pre-GA platform is unusual.

- **The underlying platform substrate has been in production use by major enterprises.** Employees at TikTok, Microsoft, Adobe, Oracle, Intel, Meta, Booking.com, Novo Nordisk, and others have used the substrate Remy now compiles against in production. Remy itself is pre-revenue and those users are not Remy customers — the list speaks to the maturity of the underlying infrastructure, not to Remy's commercial adoption. Several of those organizations are exploring Remy at alpha.

- **Reception across the broader alpha is consistent.** First-time users are routinely surprised by the quality of what Remy produces. [Video demos: https://www.youtube.com/@MindStudio_ai/videos]
...
- **Mitigation:** The output Remy produces is standard TypeScript in a standard Git repo, talking to a standard managed Postgres via a thin SDK that's itself a typed wrapper over HTTP. The user owns 100% of the code and database from day one. Porting off the platform is the same shape of work as porting any Vercel-hosted Next.js app or Heroku-hosted Rails app: rewrite the integration layer to talk to a different runtime, migrate the database, deploy elsewhere. The *runtime* is the platform's — the same way Vercel's runtime is Vercel's — but the *output* isn't proprietary. Every major cloud provider has internal services named after the company; that's runtime infrastructure, not vendor lock-in in the sense the term usually carries. For enterprise customers whose procurement requires self-hosting, the platform also supports on-premise deployment (see [Moat](#moat)).
