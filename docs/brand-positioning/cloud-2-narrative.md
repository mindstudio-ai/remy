...
Then one day, someone starts sending thousands of SMS password reset requests from a random country. The builder doesn't know why — could be a bot, could be someone probing for weaknesses, could be entirely incidental — but it doesn't matter. Each SMS costs real money, and without rate limiting, the bill grows by the minute. And that's just one shape of abuse — as the app gets more visible, automated scans, injection attempts, and DDoS attempts all start showing up too. The app needs a security layer around it: rate limits on sensitive endpoints, a web application firewall, intrusion detection, DDoS protection. That's Cloudflare or similar, $20-200/month at the entry level, more at scale.

Eventually legal asks a question that needs real records — who looked at this data, when, and why. The app needs audit logs and proper access controls, often hundreds of dollars a month for compliance-grade tooling.
...
And it's not just about replacing the existing stack. New categories keep emerging — documentation that stays current with the product, user support flows, GDPR and cookie management, marketing materials, internal communications. A new world will need new tools, too.

And there's the other side: what becomes possible when everything runs on one unified stack. When the app, its marketing, its docs, its support all share one source of truth, new things open up. Automated A/B testing on landing pages tied to actual product changes. Marketing flows that respond to what users do in the app. The kind of integration that only works when one system understands the whole product, instead of a dozen services held together by hand.
...
Remy is what that looks like — a cloud platform designed from the start for how software actually gets made now. One agent, one platform, one bill, in place of the dozen separate services someone would otherwise wire together themselves. And when one agent can see across every layer of the stack — code, data, users, traffic — and act on what it finds at agent speed, things become possible that no team coordinating across separate services could do. The same kind of shift AWS represented two decades ago — now applied to a world where agents do the building.
