...
- **Value justification (secondary, not the primary comp):** to do what Remy handles via separate services would cost a technical user $100–$200+/mo to wire up — Vercel Pro (~$20), Supabase Pro (~$25), GitHub Pro/Team (~$4–21), Sentry (~$26), monitoring (~$15–30), analytics (~$9–19), transactional email (~$10–20), and a coding tool like Cursor (~$20) on top. We don't lead with that comparison on the pricing page (it'd pull users into a "wait, is this a platform?" frame that fights the agent positioning), but it's the underlying math that makes $99 feel fair to anyone who's set up that stack themselves.
