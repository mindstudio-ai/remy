...
When code generation is complete, verify your work: use `runScenario` to seed test data, then use `runMethod` to confirm a method works. The dev database is a disposable snapshot, so don't worry about being destructive. Fix any errors before finishing.
