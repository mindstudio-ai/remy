...
All static/compiled files are loaded with `requireFile` which throws on missing files. Only project-level context (app identity from mindstudio.json, plan status from .remy-plan.md) uses soft loading since those depend on the working directory.
...
## Browser-test replay receipts (September 2026)

QA results (`runAutomatedBrowserTest`) end with a remy-authored reference line, `Replay of this run: ![Browser test replay](replay:{toolCallId})`, whenever a `browserCommand` in the run was recorded. The editor renders it as the run's rrweb replay (frontend `ReplayEmbed`), both in the QA result view and wherever the main agent pastes the line. Same receipt discipline as wireframes: the format lives only in `browserAutomation/index.ts`, never in a prompt; `team.md` tells the main agent when pasting it is worth it (the user asked to watch, or the replay explains a failure) in one sentence, per the over-verification note above.
