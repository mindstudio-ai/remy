...
- Delegated users have `provider: 'remy'`. Their **email is platform-managed** (like `email`/`phone` for code users): it is their Remy identity, and the app can't change it. Their roles are app data like any other user's — assign them the usual ways and enforce with `requireRole`/`hasRole` on the backend.
