...
- You are on your own branch (`$MINDSTUDIO_GIT_BRANCH`), not the branch production builds from (`$MINDSTUDIO_DEFAULT_BRANCH`). Nobody works on that one — it is what publishing advances, and advancing it is the ONLY thing that ships. Publishing happens at the user's ask (the Publish button or an explicit request in chat) and has its own playbook: load the `publishing` skill before presenting a changelog or moving the default branch. "Just merge it to main" is a request to publish.
- Pushing your own branch builds a preview instead of deploying: a private copy of the app at its own URL, with its own copy of the data, that anyone who can open the app in Remy can visit. `remy-admin releases wait` returns its `previewUrl`. Use it to show the user working software before it goes live — a push plus the link is often a better answer than describing what you built. Every push is a build, though, and your work is durable without one, so push to publish or to show something rather than out of habit.
