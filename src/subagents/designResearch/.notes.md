...
- **Compiled Fontshare catalog + pairings** — need to fetch the full catalog and pairings, compile into a JSON file for runtime sampling
- **Compiled design inspiration images** — need to grab Godly thumbnail URLs (via Chrome dev tools), potentially other galleries, into a JSON array for runtime sampling
- **`analyzeDesignReference` tool** — wrapper around `analyzeImage` with pre-built analysis framework prompt. Not yet implemented.
- **Stock photo search tools** — need to add Pexels/Unsplash/Pixabay search as tools (via SDK CLI, replacing searchGoogleImages)
- **Runtime sampling in prompt loader** — the mechanism to read JSON arrays, pick random subsets, and inject into the system prompt before each invocation
- **Wireframe generation prompts** — need to figure out good prompts for `generateImage` that produce useful layout wireframes
- **Layout guidance static doc** — the "what makes layouts interesting" guidelines, currently just noted in this doc, need to be written out properly for the agent's prompt
