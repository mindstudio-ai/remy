...
export async function runSubAgent(
  ...
  const run = async (): Promise<SubAgentResult> => {
    ...
    // No status watcher here, deliberately. There used to be one PER SUB-AGENT
    // RUN alongside the parent's, so a turn with two sub-agents generated three
    // labels every five seconds — roughly a third of the ~7,000 paid label
    // calls one workspace made in four days (RPT-1252). The parent's watcher
    // already describes the turn, including which sub-agent it is waiting on,
    // and this loop still emits its own model-free statuses for the states
    // worth calling out (a connection retry, a failed validation). A label per
    // agent is not worth a model call per agent.
    //
    // The try/finally that used to wrap this loop went with it: its only job
    // was stopping that watcher's interval on every exit path.
    while (true) {
      turns++;
      if (signal?.aborted) {
        return abortResult([]);
